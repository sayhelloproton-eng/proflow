...
`ACTION_REQUIRED` 必须是**可执行引导**而不是状态标签。`data` 必须通过 `ModuleSetupPlanData` 返回完整场景步骤；Step state 固定为 `DONE | TODO | BLOCKED`。已完成步骤由 Module-owned 真实状态或验证证据判断，`FAILED` 必须至少返回一个带 `blockedReason` 的 `BLOCKED` Step。每个 Step 固定包含唯一 ID、标题、简短说明、责任方、人工命令、AI 非交互命令、最小输入、verify 与 success condition；不可自动化时补充 Human Action。Platform 只校验、排序和翻译公共字段，不保存输入或步骤状态。

`Module.docs` 的 `data` 固定为 `{ docs: string }`，其中 `docs` 是 `DOCS.md` 正文，不是文件名或 documentation index；不得混入 `SETUP.md`。
## 9. Documentation

标准知识文件固定为 `DOCS.md` 与 `SETUP.md`。`docs` 返回 Module-owned knowledge；`setup` 根据真实状态给出完整、可恢复的场景步骤。Schema/descriptor 不是第三份指导文档。

`DOCS.md` 固定覆盖：模块定位与作用、主要能力、提供的 API/Public Contract、依赖的 Module/Contract/外部资源、运行形态与生命周期、使用方式、职责边界与限制、术语。正文语言为中文，必要术语采用 `English（中文释义）`；逻辑 Provides/Requires 必须与 descriptor 一致，且不得混入 Setup 操作步骤。
