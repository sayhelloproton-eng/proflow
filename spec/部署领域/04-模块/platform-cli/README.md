...
定向 `platform setup --module <moduleRef>` 仅重新观察 owning Module。真正需要的最小人工输入由 Module-owning setup CLI 以明确参数收集；Platform 不接受原始 JSON input，也不要求用户提供系统可自行确定的 path/token/endpoint/shared fact。
