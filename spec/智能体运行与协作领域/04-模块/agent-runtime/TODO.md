...
Role Registry/credential、Task-bound Worker defensive validation、Collaboration durable truth、terminal/missing/duplicate 防御与 cross-domain ports 已存在并通过 current critical proofs。

## Historical task identities

历史任务标识 `AGT-RUNTIME-001..006` 已从 current backlog 移除。Test Plan 中若仍引用这些 ID，它们只作为历史 traceability/provenance identity，不表示当前存在待实施工作；后续文档治理 Wave 会统一检查 dangling/重复导航。
