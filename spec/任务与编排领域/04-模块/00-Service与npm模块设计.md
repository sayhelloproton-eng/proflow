...
Task Domain v1 由独立 npm package 提供，并由 `@tomflow/proflow-platform-host` 作为 Application Composition Root **in-process 装配**。Task 不单独建立 `task-service` daemon；若未来出现真实独立部署需求，再复用相同 Public Contract 和 Service 层。
...
由 `@tomflow/proflow-task-orchestration` 统一提供。
...
# 7. @tomflow/proflow-task-orchestration
...
# 8. @tomflow/proflow-task-store-sqlite
...
未来多个领域真实复用后，再考虑抽 `@tomflow/proflow-migration-runner`。
...
- `@tomflow/proflow-task-orchestration`、`task-store-sqlite`、`task-migration-runner` 继续保持独立 npm package；不得把 Task 源码直接内嵌到 `platform-host`。
