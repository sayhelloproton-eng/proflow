...
- [ ] **CP-TASK-ORCH-05** — `reopenNode` 保留旧 execution history 与稳定 TaskRoleBinding/Conversation；目标 Node `runNo + 1` 且目标及后续 Node 的 run-level `workerRef` 清空；任何已开始过但被回退失效的后续 Node 必须先归档旧 run 并 `runNo + 1`，从未开始的后续 Node 不虚增 generation；下一次 `startNode` 从同一 TaskRoleBinding 重新解析原 Worker，Task currentNodeId 回到目标 Node，后续 rerun 不得复用已归档 `(nodeId, runNo)`。
...
- [ ] **RF-TASK-ORCH-05** — reopen 覆盖旧 history/TaskRoleBinding，Node run-level workerRef 未按规则清空/重解析，目标或已开始后续 Node 复用已归档 runNo、从未开始后续 Node 虚增 runNo，或 currentNodeId/后续 Node 重置错误
