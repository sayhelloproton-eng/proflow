...
`.proflow` 不能整体忽略，因为里面既有正式 Markdown 工作产物，也有运行时数据库。
...
当前 `/Users/agent/Desktop/proflow` 既是现阶段源码所在 Git 仓库，也是未来 Task Domain 完成后可以拿来真实 dogfooding 的目标仓库。

ProFlow 正式技术方案仍写入当前源码目录正常的 `docs/technical/技术方案/ProFlow/**`；Task Runtime 运行中产生的 Task-scoped working documents / state 遵循 `.proflow/**`。
