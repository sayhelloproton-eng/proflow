...
2. `15-当前上下文与Tunnel任务流转-20260825.md` 顶部最新覆盖层 —— 获取当前收口快照与最新执行策略；后续历史快照仍需机械核验。
3. `05-执行纪律与工具规则.md` —— 明确 CodeGraph-first、先审完冻结 residual list、统一整改/统一验证、实时汇报等当前纪律。
4. `04-测试验证与验收方法.md` —— 再确定本轮如何验收、如何分类失败、什么才算 PASS。
5. `03-冻结架构与关键决策.md` —— 防止为测试方便重开 Frozen Architecture。
6. `14-配置自动化与OpenAI-Secure-MCP-Tunnel后续.md` —— 配置自动化审计、技术问答、人工边界与当前自动化停止门。
...
阶段状态变化优先更新 `02`；验收方法/永久纪律变化更新 `05`；当前执行策略或收口快照变化同步更新 `15` 顶部覆盖层，避免新建重复交接文档。
