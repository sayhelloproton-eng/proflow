...
if (!workspaceArg) {
  console.error("Usage: pnpm fresh:workspace --workspace <absolute-path> [--dry-run]");
  process.exit(2);
}

const workspace = realpathSync(resolve(workspaceArg));
const repositoryRoot = realpathSync(resolve(import.meta.dirname, ".."));
if (workspace === "/" || workspace === homedir() || workspace === repositoryRoot) {
  throw new Error(`Refusing unsafe workspace: ${workspace}`);
}

const protectedRepo = join(workspace, "repos", "proflow");
const packageJsonPath = join(workspace, "package.json");
const hasKnownMarker = existsSync(join(workspace, ".proflow")) ||
  existsSync(join(workspace, "node_modules", "@tomflow", "proflow-platform-cli")) ||
  existsSync(protectedRepo);
if (!hasKnownMarker) throw new Error("Refusing workspace without a ProFlow marker");
...
for (const target of cleanupTargets) {
  ...
  if (rel.startsWith("..") || rel === "") throw new Error(`Unsafe cleanup target: ${target}`);
}
...
if (!skipStop && existsSync(platformBin)) {
  if (dryRun) {
    ...
    if (stopped.status !== 0) throw new Error(`platform stop failed with status ${stopped.status}`);
  }
}
...
if (!dryRun) {
  ...
  if (leftovers.length > 0) throw new Error(`Fresh hygiene failed: ${leftovers.join(", ")}`);
}

console.log(`${dryRun ? "Fresh Workspace DRY-RUN PASS" : "Fresh Workspace READY"}: ${workspace}`);
console.log(`Protected: ${existsSync(join(workspace, "repos")) ? "repos/" : "no repos/ directory"}`);
