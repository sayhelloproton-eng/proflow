...
for (const request of requested) {
	const pkg = packages.find(
		(candidate) => candidate.dirName === request || candidate.name === request,
	);
	if (!pkg) {
	...
	for (const required of ["test", "typecheck"]) {
		if (!pkg.scripts[required]) {
			console.error(
				`${pkg.name} is missing required package gate script: ${required}`,
			);
			process.exit(2);
		}
	}
	...
	execFileSync("pnpm", ["--dir", pkg.directory, "typecheck"], {
		stdio: "inherit",
	});
	if (pkg.scripts.lint) {
		execFileSync("pnpm", ["--dir", pkg.directory, "lint"], {
			stdio: "inherit",
		});
	}
}
