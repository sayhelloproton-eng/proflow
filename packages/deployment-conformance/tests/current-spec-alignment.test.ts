...
test("SPEC-ALIGN deployment-conformance remains registered and has an ACTIVE current Module Test Plan", async () => {
	const registry = JSON.parse(await readFile(moduleRegistryUrl, "utf8")) as Array<{ moduleRef?: string }>;
	const matches = registry.filter((entry) => entry.moduleRef === "deployment-conformance");
	...
	const plan = index.documents.find((entry) => entry.moduleRef === "deployment-conformance");
	...
	assert.equal(plan.path, "部署领域/07-测试计划/modules/deployment-conformance.md");
