...
export const moduleStatusObservationSchema = z
    ...
    if (observation.setupStatus !== "READY" &&
    ...
    if (observation.runtimeStatus === "FAILED" &&
...
export const moduleSetupStepSchema = z
    ...
    requiredInputs: z.array(z.strictObject({
        name: identifier,
        description: z.string().min(1),
        sensitive: z.boolean(),
    })),
...
export const moduleManagementCommandSchema = z.enum(standardModuleManagementCommands);
...
export function parseModuleDescriptor(input) {
    return moduleDescriptorSchema.parse(input);
}
export function queryRequirements(descriptor) {
...
function versionParts(version) {
    const [majorPart = "0", minorPart = "0", patchPart = "0"] = version.split(".");
    return [
...
function rangeIsTighter(currentRange, targetRange) {
...
export function assessModuleCompatibility(current, target) {
    const breakingChanges = [];
    if (current.moduleRef !== target.moduleRef ||
        current.packageName !== target.packageName ||
        current.kind !== target.kind) {
        breakingChanges.push("module identity or kind changed");
    }
    if (current.identity.domain !== target.identity.domain ||
    ...
    for (const provided of current.provides) {
        const replacement = target.provides.find((item) => item.contractRef === provided.contractRef);
        if (replacement === undefined) {
            breakingChanges.push(`provided contract removed: ${provided.contractRef}`);
        }
        else if (major(replacement.version) !== major(provided.version) ||
            compareVersions(replacement.version, provided.version) < 0) {
            breakingChanges.push(`provided contract major version changed: ${provided.contractRef}`);
        }
    }
    for (const required of target.requires) {
        const previous = current.requires.find((item) => item.contractRef === required.contractRef);
        if (previous === undefined && required.optional !== true) {
            ...
            breakingChanges.push(`optional contract became required: ${required.contractRef}`);
        }
        else if (previous !== undefined &&
            rangeIsTighter(previous.versionRange, required.versionRange)) {
            breakingChanges.push(`required contract range tightened: ${required.contractRef}`);
        }
    }
    ...
    if (JSON.stringify(current.documentation) !==
        JSON.stringify(target.documentation)) {
        breakingChanges.push("module documentation entries changed");
    }
    if (rangeIsTighter(current.platformCompatibility, target.platformCompatibility)) {
