...
test("SPEC-ALIGN agent-product remains registered and has an ACTIVE current Module Test Plan", async () => {
	...
	const matches = registry.filter((entry) => entry.moduleRef === "agent-product");
	...
	const plan = index.documents.find((entry) => entry.moduleRef === "agent-product");
	...
	assert.equal(plan.path, "智能体运行与协作领域/07-测试计划/modules/agent-product.md");
