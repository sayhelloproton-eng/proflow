...
test("SPEC-ALIGN task-migration-runner remains registered and has an ACTIVE current Module Test Plan", async () => {
	const registry = JSON.parse(await readFile(moduleRegistryUrl, "utf8")) as Array<{ moduleRef?: string }>;
	const matches = registry.filter((entry) => entry.moduleRef === "task-migration-runner");
	...
	const plan = index.documents.find((entry) => entry.moduleRef === "task-migration-runner");
	...
	assert.equal(plan.path, "任务与编排领域/07-测试计划/modules/task-migration-runner.md");
