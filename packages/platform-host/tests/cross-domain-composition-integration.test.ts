...
test("REG-HOST-COMPOSITION-01 formal owner packages stay independent while Host only composes public paths", () => {
	...
	]) assert.equal(packageJson.dependencies?.[dependency] !== undefined, true, dependency);
	for (const service of [
		"@tomflow/proflow-execution-runtime",
		"@tomflow/proflow-model-runtime",
	]) assert.equal(packageJson.devDependencies?.[service] !== undefined, true, service);
	assert.doesNotMatch(source, /createExecutionRuntime\(|createModelRuntimeProcess\(|createAgentGatewayProcess\(/);
});
