...
test("SPEC-ALIGN platform-host remains registered and has an ACTIVE current Module Test Plan", async () => {
	const registry = JSON.parse(await readFile(moduleRegistryUrl, "utf8")) as Array<{ moduleRef?: string }>;
	const matches = registry.filter((entry) => entry.moduleRef === "platform-host");
	...
	const plan = index.documents.find((entry) => entry.moduleRef === "platform-host");
	...
	assert.equal(plan.path, "平台架构与公共约定/06-测试计划/modules/platform-host.md");
