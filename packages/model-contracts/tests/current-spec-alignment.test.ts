...
test("SPEC-ALIGN model-contracts remains registered and has an ACTIVE current Module Test Plan", async () => {
	const registry = JSON.parse(await readFile(moduleRegistryUrl, "utf8")) as Array<{ moduleRef?: string }>;
	const matches = registry.filter((entry) => entry.moduleRef === "model-contracts");
	...
	const plan = index.documents.find((entry) => entry.moduleRef === "model-contracts");
	...
	assert.equal(plan.path, "模型与推理领域/07-测试计划/modules/model-contracts.md");
