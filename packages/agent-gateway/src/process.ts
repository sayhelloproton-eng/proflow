...
export async function createAgentGatewayProcess(input: {
	...
	const credentials = record(
		JSON.parse(await readFile(input.config.credentialFile, "utf8")),
		"credential store",
	);
	for (const [roleRef, credential] of Object.entries(credentials))
		if (
			roleRef.length === 0 ||
		...
		owners: {
			...
			async readiness() {
				try {
					...
					return {
						ingress: true,
						credentialStore: Object.keys(credentials).length > 0,
