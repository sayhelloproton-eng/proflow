...
test("CP-AGT-TEST-07 J1 Test/Ops is bind-only/IDLE and starts the formal Node only after NODE_READY wake", () => {
	assert.match(
		...
		/WORKER_BIND.*(?:不工作|只绑定|不推进)/,
	);
	assert.ok(operationIds.includes("startNode"));
	assert.doesNotMatch(
		JSON.stringify(openapi),
		/worker\.create|createConversation/,
	);
});

test("CP-AGT-TEST-08 one Worker Turn permits 0..N routine Actions and ships all Direct Tool metadata nonconsequential", () => {
...
test("CP-AGT-TEST-10 fail→reopen keeps the same Worker/Conversation and runNo advances through Task ownership", () => {
	...
	assert.doesNotMatch(
		JSON.stringify(openapi),
		/worker\.create|createConversation|replaceWorker/,
	);
