...
test("SPEC-ALIGN agent-test-ops remains registered and has an ACTIVE current Module Test Plan", async () => {
	const registry = JSON.parse(await readFile(moduleRegistryUrl, "utf8")) as Array<{ moduleRef?: string }>;
	const matches = registry.filter((entry) => entry.moduleRef === "agent-test-ops");
	...
	const plan = index.documents.find((entry) => entry.moduleRef === "agent-test-ops");
	...
	assert.equal(plan.path, "智能体运行与协作领域/07-测试计划/modules/agent-test-ops.md");
