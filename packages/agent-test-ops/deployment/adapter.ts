								...
								issues: [
									{
										...
										nextCommand:
											missingPrerequisites.length > 0
												? "platform setup"
												: "platform setup --module agent-test-ops",
									},
								],
							}),
				},
			...
			return {
				result: {
					...
					actionRequired: {
						...
						description:
							"Existing Custom GPT Role is drifted. Automatic setup does not edit an existing GPT. After operator/browser synchronizes and verifies the current package material, run: proflow-agent-test-ops role adopt <current-carrier-url> --workspace <workspace>.",
					},
				},
				observedEffects: [] as string[],
			};
