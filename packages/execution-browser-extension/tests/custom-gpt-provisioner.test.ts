...
test("CP-EXE-BR-19 role credential is handed to Extension only for one Auth finalization command", async (context) => {
	const host = await createCustomGptProvisioningHost({
		token,
		extensionId,
		commandTimeoutMs: 2_000,
	...
	context.after(() => host.close());
	await extensionFetch(host.endpoint, "/v1/provisioning/session/hello", {
		method: "POST",
		body: JSON.stringify({ extensionId, extensionInstanceId }),
	});
	...
	const credential = "role-scoped-bearer-credential-0123456789";
	const pending = host.finalizeRoleAuth({
		carrierUrl: "https://chatgpt.com/g/g-product",
		credential,
	});
	const nextPath = `/v1/provisioning/commands/next?extensionInstanceId=${encodeURIComponent(extensionInstanceId)}`;
	...
	while (next.status === 204 && Date.now() < deadline) {
		await new Promise<void>((resolveWait) => setTimeout(resolveWait, 25));
		next = await extensionFetch(host.endpoint, nextPath);
	}
	assert.equal(next.status, 200);
	...
	const report = await extensionFetch(
		...
		`/v1/provisioning/commands/result?extensionInstanceId=${encodeURIComponent(extensionInstanceId)}`,
		{
			...
			body: JSON.stringify({
				commandId: command.commandId,
				ok: true,
				value: { status: "AUTH_UPDATED", gptId: "g-product" },
			}),
		},
	);
	assert.equal(report.status, 200);
	...
	assert.equal((await extensionFetch(host.endpoint, nextPath)).status, 204);
	assert.doesNotMatch(JSON.stringify(host.status()), /role-scoped-bearer/);
});
