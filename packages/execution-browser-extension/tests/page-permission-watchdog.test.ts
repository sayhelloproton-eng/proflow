...
test("CP-EXE-BR-22 permission watchdog runs every ten seconds", () => {
	assert.equal(PAGE_PERMISSION_WATCHDOG_INTERVAL_MS, 10_000);
});

test("CP-EXE-BR-22 bottom generic 拒绝/允许 controls trigger a permission refresh", () => {
	assert.equal(
...
test("CP-EXE-BR-22 bottom 始终允许/允许一次 variants remain detectable", () => {
	assert.equal(
...
test("CP-EXE-BR-22 unrelated or incomplete bottom controls do not trigger", () => {
...
test("CP-EXE-BR-22 watchdog is intentionally bounded to the page-bottom button window", () => {
