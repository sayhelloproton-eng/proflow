...
test("CP-EXE-BR-18 extension wires provisioning bridge to the GPT editor content surface", async () => {
	...
	assert.match(content, /savedConfirmationVisible/);
	...
	assert.doesNotMatch(content, /GPT_EDITOR_KNOWLEDGE_CONTENT_UNVERIFIED/);
	assert.match(content, /knowledgeRemoveControl/);
	assert.match(content, /Remove file:/);
	assert.match(content, /移除文件：/);
	assert.match(content, /GPT_EDITOR_KNOWLEDGE_REMOVE_CONTROL_NOT_FOUND/);
	assert.match(content, /GPT_EDITOR_KNOWLEDGE_REMOVE_TIMEOUT/);
	const uploadStart = content.indexOf("async function uploadKnowledge");
	const uploadEnd = content.indexOf("const privateVisibilityLabels", uploadStart);
	assert.ok(uploadStart >= 0 && uploadEnd > uploadStart);
	const upload = content.slice(uploadStart, uploadEnd);
	const fetched = upload.indexOf("await fetchKnowledgeRelay(descriptor.url)");
	const sized = upload.indexOf("bytes.byteLength !== descriptor.sizeBytes");
	const hashed = upload.indexOf("await sha256Bytes(bytes)");
	const removed = upload.indexOf("await removeExistingKnowledge(descriptor.name)");
	assert.ok(fetched >= 0 && fetched < sized && sized < hashed && hashed < removed);
