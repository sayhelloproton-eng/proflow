...
test("CP-EXE-RT-22 Browser/local effects converge to one durable Execution truth", async () => {
	...
	assert.doesNotMatch(text, /CREATE TABLE IF NOT EXISTS (?:browser_effects|observer_effects|carrier_effects)/);
...
test("CP-EXE-RT-09 File Bridge locator materialization produces Artifact truth before reuse", async () => {
	...
	assert.doesNotMatch(runtime, /locator.*(?:SUCCEEDED|business mutation).*true/i);
});
...
test("CP-EXE-RT-10 Context Pack/Patch remain Artifact subtypes and patch apply is a separate effect", async () => {
	...
	assert.doesNotMatch(text, /CREATE TABLE IF NOT EXISTS (?:context_packs|patches)/i);
...
test("CP-EXE-RT-11 model/observer judgment cannot bypass hard Execution authority", async () => {
	...
	assert.ok(modelIndex >= 0, "Model decision must remain an optional subordinate step");
	assert.ok(hardPolicyIndex < modelIndex, "hard policy must run before model inference");
	assert.match(text, /approval/i);
	assert.doesNotMatch(text, /confidence\s*[>=]+.*(?:ALLOW|SUCCEEDED)/i);
	assert.doesNotMatch(text, /(?:TaskDiagnostic|SystemAssessment).*executeCapability/i);
});
