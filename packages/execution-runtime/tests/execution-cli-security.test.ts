...
test("RF-EXE-RT-11 formal execution CLI rejects group/world-readable transport credentials", async (t) => {
	...
	await writeFile(identity, "identity-credential-value-abcdefghijklmnopqrstuvwxyz\n", { mode: 0o600 });
	await writeFile(transport, "execution-credential-value-abcdefghijklmnopqrstuvwxyz\n", { mode: 0o600 });
	await writeFile(model, "model-credential-value-abcdefghijklmnopqrstuvwxyz\n", { mode: 0o600 });
	await chmod(transport, 0o644);
	const config = join(root, "config.json");
	await writeFile(config, JSON.stringify({
		databasePath: join(root, "execution.sqlite"), projectRoot: root, artifactRoot: join(root, "artifacts"), host: "127.0.0.1", port: 0, exactNetworkTargets: [],
		browserExecutorConfigPath: join(root, "browser.json"), transportCredentialFile: transport,
	...
	assert.match(result.stderr, /execution transport credential permissions must be owner-only/);
});
