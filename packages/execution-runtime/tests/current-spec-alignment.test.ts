...
test("SPEC-ALIGN execution-runtime remains registered and has an ACTIVE current Module Test Plan", async () => {
	const registry = JSON.parse(await readFile(moduleRegistryUrl, "utf8")) as Array<{ moduleRef?: string }>;
	const matches = registry.filter((entry) => entry.moduleRef === "execution-runtime");
	...
	const plan = index.documents.find((entry) => entry.moduleRef === "execution-runtime");
	...
	assert.equal(plan.path, "执行领域/07-测试计划/modules/execution-runtime.md");
