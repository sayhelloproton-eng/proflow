...
test("SPEC-ALIGN platform-cli remains registered and has an ACTIVE current Module Test Plan", async () => {
	const registry = JSON.parse(await readFile(moduleRegistryUrl, "utf8")) as Array<{ moduleRef?: string }>;
	const matches = registry.filter((entry) => entry.moduleRef === "platform-cli");
	...
	const plan = index.documents.find((entry) => entry.moduleRef === "platform-cli");
	...
	assert.equal(plan.path, "部署领域/07-测试计划/modules/platform-cli.md");
