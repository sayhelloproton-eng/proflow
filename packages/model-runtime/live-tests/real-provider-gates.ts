...
const underlying = createOpenAICompatibleProvider({
	...
	roleBody: {
		fast: { temperature: 0 },
		reason: { temperature: 0, top_p: 0.8 },
	},
	roleSystemPrompt: {
...
const runtime = createModelRuntime({
	specs: [systemHealthAssessmentSpec, visionSpec, proposalSpec],
	roles,
	provider: trackedProvider,
	inferenceTimeoutMs: 360_000,
});
...
if (m4Results.some((result) => result.status !== "SUCCEEDED"))
	throw new Error("M4 mixed sustained run contained a failure");
