...
test("SPEC-ALIGN model-runtime remains registered and has an ACTIVE current Module Test Plan", async () => {
	const registry = JSON.parse(await readFile(moduleRegistryUrl, "utf8")) as Array<{ moduleRef?: string }>;
	const matches = registry.filter((entry) => entry.moduleRef === "model-runtime");
	...
	const plan = index.documents.find((entry) => entry.moduleRef === "model-runtime");
	...
	assert.equal(plan.path, "模型与推理领域/07-测试计划/modules/model-runtime.md");
